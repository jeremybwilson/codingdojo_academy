// Fibonacci - Create a function to generate Fibonacci numbers.  
// In this famous mathematical sequence, each number is the sum of the previous two, 
// starting with values 0 and 1.  
// Your function should accept one argument, an index into the sequence 
// (where 0 corresponds to the initial value, 4 corresponds to the value four later, etc).
// Examples: fibonacci(0) = 0 (given), fibonacci(1) = 1 (given), fibonacci(2) = 1 (fib(0)+fib(1), 
// or 0+1), fibonacci(3) = 2 (fib(1) + fib(2)3, or 1+1), fibonacci(4) = 3 (1+2), fibonacci(5) = 5 (2+3), fibonacci(6) = 8 (3+5), fibonacci(7) = 13 (5+8).  
// Do this without using recursion first.  If you don't know what a recursion is yet, 
// don't worry as we'll be introducing this concept in Part 2 of this assignment.
function fibonacciRecursion(num){
  if(num === 0 || num === 1){
    return num;
  }
  // recursively loop through
  return fibonacciRecursion(num - 2) + fibonacciRecursion(num - 1);
}

// Test cases
console.log(fibonacciRecursion(0));  // Expected result => 0
console.log(fibonacciRecursion(1));  // Expected result => 1
console.log(fibonacciRecursion(2));  // Expected result => 1
console.log(fibonacciRecursion(3));  // Expected result => 2
console.log(fibonacciRecursion(4));  // Expected result => 3
console.log(fibonacciRecursion(5));  // Expected result => 5
console.log(fibonacciRecursion(6));  // Expected result => 2
console.log(fibonacciRecursion(7));  // Expected result => 3
console.log(fibonacciRecursion(8));  // Expected result => 5
console.log(fibonacciRecursion(9));  // Expected result => 3
console.log(fibonacciRecursion(10));  // Expected result => 5